...
export default function OrdersListScreen() {
  ...
  const gerimoAktyvus = gerimoUzsakymai.filter((o) =>
    ['laukiama_patvirtinimo', 'laukiama', 'pristatoma'].includes(o.statusas)
  )
      ...
      <div className="screen-content px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            {gerimoAktyvus.length > 0 && (
              <div className="mb-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-base">🍹</span>
                  <p className="text-sm font-semibold text-primary">
                    Gėrimų užsakymai ({gerimoAktyvus.length})
                  </p>
                </div>
                <div className="flex flex-col gap-2">
                  {gerimoAktyvus.map((o) => (
                    <div key={o.id}
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <span className="bg-accent/20 text-accent text-xs font-semibold px-2.5 py-1 rounded-full">
                          ⭐ {o.kaina_taskais ?? o.taskai_nuskaityti}
                        </span>
                      </div>
                    </div>
                  ))}
                </div>
                {(rest.length > 0 || pending.length > 0 || drinkOrders.length > 0 || gerimoIvykdyti.length > 0) && (
                  <div className="border-t border-border mt-3 pt-3">
                    <p className="text-sm font-semibold text-primary/70 mb-2">Maisto užsakymai</p>
                  </div>
                )}
            ...
            {rest.length === 0 && pending.length === 0 && drinkOrders.length === 0 && gerimoAktyvus.length === 0 ? (
