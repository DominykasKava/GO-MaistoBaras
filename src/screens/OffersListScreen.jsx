...
function LiveMap({ center, markers = [], onMarkerClick }) {
  ...
  return (
    <GoogleMap mapContainerStyle={{ width: '100%', height: '220px' }} center={center} zoom={13}>
      {markers.map((m) =>
        m.lat && m.lng ? (
...
function TransportuotojasView() {
  ...
  const reload = useCallback(async () => {
    try { setOrders(await getOrders() ?? []) } catch {}
  }, [])
  ...
  const goRoute = (o) => {
    const from = o.pickup_address
      ?? (o.offer_lat && o.offer_lng ? `${Number(o.offer_lat)},${Number(o.offer_lng)}` : null)
    const to = o.delivery_address
      ?? (o.delivery_lat && o.delivery_lng ? `${Number(o.delivery_lat)},${Number(o.delivery_lng)}` : null)
    if (!from || !to) return
  ...
  const handleAccept = async (id) => {
    ...
    try {
      ...
      await reload()
    } catch (err) { showToast(err.response?.data?.message ?? 'Klaida', 'error') }
          ...
          {mano.length > 0 && (
            <div>
              ...
              <div className="flex flex-col gap-3">
                {mano.map((o) => (
                  <div key={o.id} className="rounded-2xl border border-accent/20 bg-accent/10 p-4">
                    ...
                    <div className="flex gap-2 mt-3">
                      <button
                        onClick={() => goRoute(o)}
                        className="flex-1 py-2 rounded-xl text-xs font-semibold bg-primary/10 text-primary border border-primary/20"
                      >
                        🗺️ Maršrutas
                      </button>
                      <button
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-3">
                {laukiama.map((o) => {
                  ...
                  return (
                    <div key={o.id} className={`rounded-2xl border shadow-sm p-4 ${farAway ? 'border-border bg-surface opacity-60' : 'border-border bg-white'}`}>
                      ...
                      <div className="mt-3">
                        <button
                          onClick={() => goRoute(o)}
                          className="w-full py-2 rounded-xl text-xs font-semibold bg-primary/10 text-primary border border-primary/20 mb-2"
                        >
                          🗺️ Rodyti maršrutą
                        </button>
